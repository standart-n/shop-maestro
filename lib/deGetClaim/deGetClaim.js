	...
	de_site=$('#'+de.siteId).val();
		$.ajax({
			...
			success:function(answer){
				...
				if (de.enterEmail!="FALSE") { if (de_email!="") { de.msg+=' email: '+de_email; } }
				...
				if (de_city!=undefined) {
					...
					if (de_city!="") { de.msg+=' карта: http://maps.yandex.ru/?text='+de_city; }
				}
				$.ajax({
					url:"http://www.standart-n.ru/api/icq/",
					type:"GET",
					data:{ 
						uin:de.icq,
						msg:de.msg
...
function getClaim_init() {
    ...
    if (de.imgPath==undefined) {de.imgPath="http://www.dement.ru/!lib/deGetClaim/img/";}    
